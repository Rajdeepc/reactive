...
const Card = (props) => (
    <div>
        <div style={{ paddingTop: '20px', paddingBottom: '20px', background: 'white'}}>
            <h3 style={{ fontSize: '25.2px', marginBottom: 0, lineHeight: '30px', fontWeight: 900 }}><a target="_blank" rel="noopener noreferrer" href={`https://${props.item.url}`}>{props.item.title}</a></h3>
            ...
            <small>Date Created: {props.item.created ? CustomDate(props.item.created) : ''}</small>
        </div>
    </div>
)
